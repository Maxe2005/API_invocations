...
/**
 * Validator for @IntRange annotation. Resolves min/max from NumericConstraintsConfig based on
 * constraint type, or uses custom min/max values if type is CUSTOM.
 */
    ...
    if (!isValid) {
      ...
      String message =
          String.format(
              ...
              fieldName.isEmpty() ? "Value" : fieldName, minValue, maxValue, value);
