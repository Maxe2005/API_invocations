...
public class MonsterImportExportService {
  ...
  private List<SignedUrlRequest> buildSignedUrlRequests(Map<String, MonsterEntity> withImage) {
    return withImage.values().stream()
        .map(m -> new SignedUrlRequest(m.getId(), m.getImageUrl()))
        .toList();
  }
        ...
        for (var r : responses) {
          ...
          if (signed != null && !signed.isBlank()) {
            try {
              ...
              log.info("Could not fetch signed image {} : {}", signed, ex.getMessage());
            }
          }
        }
      } catch (Exception ex) {
  ...
  private void writeZip(
      ...
      Map<String, byte[]> imageBytesById,
      Map<String, String> imageNameById)
      throws IOException {
    try (ZipOutputStream zos = new ZipOutputStream(out)) {
      for (MonsterEntity monster : monsters) {
        String folder =
            sanitizeFolder(monster.getName() == null ? monster.getId() : monster.getName());

        byte[] jsonBytes =
            objectMapper.writeValueAsBytes(dtoMapper.toGlobalMonsterWithIdDto(monster, true));
  ...
  private void tryFallbackAndWrite(ZipOutputStream zos, MonsterEntity monster) {
    ...
    for (String url : candidates) {
      try {
        ...
        log.info("IO error writing fallback image for {}: {}", monster.getId(), e.getMessage());
      }
    }
  ...
  public int importMonstersFromStream(InputStream is) throws IOException {
      ...
      while ((entry = zis.getNextEntry()) != null) {
        ...
        while ((len = zis.read(buffer)) != -1) {
          ...
          if (entryBytes > importProperties.getMaxEntryUncompressedBytes()) {
            throw new IllegalArgumentException(
                "Archive rejetée : l'entrée '"
                    + name
                    + "' dépasse la taille décompressée maximale autorisée par fichier ("
                    + importProperties.getMaxEntryUncompressedBytes()
                    + " octets)");
          }
          ...
          if (totalUncompressedBytes > importProperties.getMaxTotalUncompressedBytes()) {
            throw new IllegalArgumentException(
                "Archive rejetée : taille décompressée cumulée supérieure à la limite autorisée ("
                    + importProperties.getMaxTotalUncompressedBytes()
                    + " octets)");
          }
    ...
    for (Map.Entry<String, byte[]> e : jsonMap.entrySet()) {
      try {
        ...
        log.info("Skipping malformed monster json for folder {}: {}", e.getKey(), ex.getMessage());
      }
    }
