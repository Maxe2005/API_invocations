...
public class StatsController {
    ...
    @Operation(
            ...
            description = "Vérifie que la somme des taux de drop de tous les rangs est égale à 100%. " +
    ...
    @Operation(
            ...
            description = "Récupère les taux de drop théoriques (configurés) et/ou réels (observés) des invocations par rang. " +
                    "Permet de comparer la configuration avec les statistiques réelles du jeu.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Taux de drop récupérés avec succès",
                    content = @Content(mediaType = "text/plain", schema = @Schema(implementation = String.class))),
            @ApiResponse(responseCode = "401", description = "Non authentifié", content = @Content)
    })
    ...
    public ResponseEntity<String> getLootRatesString(
            @Parameter(
                    description = "Type de taux à retourner: 'all' (théoriques et réels), 'Theoretical Drop Rates' (configurés), ou 'Real Drop Rates' (observés)",
                    schema = @Schema(
                            type = "string",
