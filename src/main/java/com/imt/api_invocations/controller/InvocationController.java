...
@Tag(name = "Invocations", description = "API de gestion des invocations de monstres - Système de gacha avec taux de drop par rang")
public class InvocationController {
    ...
    @Operation(
            ...
            description = "Effectue une invocation aléatoire d'un monstre basée sur les taux de drop configurés par rang. " +
                    "Retourne un monstre sans ID (non sauvegardé en base).")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Monstre invoqué avec succès",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = GlobalMonsterDto.class))),
            @ApiResponse(responseCode = "401", description = "Non authentifié", content = @Content),
            @ApiResponse(responseCode = "500", description = "Erreur lors de l'invocation", content = @Content)
    })
    @GetMapping("invoque")
    ...
    @Operation(
            ...
            description = "Effectue une invocation aléatoire pour un joueur spécifique. Le monstre est sauvegardé en base de données " +
                    "et associé au joueur. Retourne le monstre avec son ID et toutes ses compétences.")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Monstre invoqué et sauvegardé avec succès",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = GlobalMonsterWithIdDto.class))),
            @ApiResponse(responseCode = "401", description = "Non authentifié", content = @Content),
    ...
    public ResponseEntity<GlobalMonsterWithIdDto> globalInvoque(
            @Parameter(description = "Identifiant unique du joueur", required = true, example = "player123")
    ...
    @Operation(
            ...
            description = "Rejoue toutes les invocations qui étaient en attente dans le buffer (pattern SAGA). " +
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Invocations rejouées avec succès",
                    content = @Content(mediaType = "application/json", schema = @Schema(implementation = InvocationReplayResponse.class))),
            @ApiResponse(responseCode = "401", description = "Non authentifié", content = @Content),
            @ApiResponse(responseCode = "500", description = "Erreur lors du rejeu des invocations", content = @Content)
    })
