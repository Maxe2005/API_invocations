...
public class SkillsController {
    ...
    @Operation(summary = "Créer une nouvelle compétence",
    ...
    public ResponseEntity<String> createSkill(
            @Parameter(description = "Données de la compétence à créer",
                    required = true) @Valid @RequestBody SkillsHttpDto skillsHttpDto) {
        String skillId = skillsService.createSkill(dtoMapper.toSkillEntity(skillsHttpDto));
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Compétence trouvée",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(implementation = SkillsWithIdDto.class))),
    ...
    public ResponseEntity<SkillsWithIdDto> getSkillById(
            @Parameter(description = "Identifiant unique de la compétence", required = true,
                    example = "507f1f77bcf86cd799439011") @PathVariable String skillId) {
    ...
    @Operation(summary = "Obtenir les compétences d'un monstre",
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404",
                    description = "Aucune compétence trouvée pour ce monstre", content = @Content),
            @ApiResponse(responseCode = "401", description = "Non authentifié",
                    content = @Content)})
    @GetMapping("/monster/{monsterId}")
    public ResponseEntity<List<SkillsWithIdDto>> getSkillByMonsterId(
            @Parameter(description = "Identifiant unique du monstre", required = true,
    ...
    public ResponseEntity<Void> updateSkill(
            @Parameter(description = "Identifiant unique de la compétence", required = true,
                    example = "507f1f77bcf86cd799439011") @PathVariable String skillId,
            @Parameter(description = "Données de mise à jour de la compétence",
                    required = true) @Valid @RequestBody SkillsHttpUpdateDto skillsUpdateDto) {
        var existingSkill = skillsService.getSkillById(skillId);
        if (existingSkill == null) {
            throw new ResourceNotFoundException("Skill", skillId);
        }
        var updatedSkill = dtoMapper.updateSkillEntity(existingSkill, skillsUpdateDto);
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "Compétence non trouvée",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(example = "{\"deleted\": false}"))),
            @ApiResponse(responseCode = "401", description = "Non authentifié",
                    content = @Content)})
    @DeleteMapping("/{skillId}")
    public ResponseEntity<Map<String, Boolean>> deleteSkillById(@Parameter(
            description = "Identifiant unique de la compétence à supprimer", required = true,
            example = "507f1f77bcf86cd799439011") @PathVariable String skillId) {
    ...
    @Operation(summary = "Supprimer les compétences d'un monstre",
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Compétences supprimées avec succès",
                    content = @Content(mediaType = "application/json",
                            schema = @Schema(example = "{\"deletedCount\": 5}"))),
            @ApiResponse(responseCode = "401", description = "Non authentifié",
                    content = @Content)})
    @DeleteMapping("/monster/{monsterId}")
    public ResponseEntity<Map<String, Long>> deleteSkillByMonsterId(@Parameter(
            description = "Identifiant unique du monstre dont supprimer les compétences",
            required = true, example = "507f1f77bcf86cd799439011") @PathVariable String monsterId) {
