...
class MonsterServiceTest {
    ...
    @Mock
    private MonsterServiceMapper monsterServiceMapper;

    @InjectMocks
    private MonsterService monsterService;

    private MonsterEntity sampleMonster() {
        return MonsterEntity.builder().name("Pyrolosse").element(Elementary.FIRE).rank(Rank.COMMON)
                .build();
    }
    ...
    void should_SaveSkills_When_MonsterHasSkills() {
        SkillEntity skill = SkillEntity.builder().name("Griffe").damage(10).cooldown(1).lvlMax(5)
                .rank(Rank.COMMON).build();
        MonsterEntity monster = MonsterEntity.builder().name("Pyrolosse").element(Elementary.FIRE)
                .rank(Rank.COMMON).skills(List.of(skill)).build();
        when(monsterRepository.save(monster)).thenReturn("monster-1");
    ...
    void should_MapBeforeUpdating_When_UpdatingMonster() {
        ...
        MonsterEntity mapped = MonsterEntity.builder().id("m-1").name("Pyrolosse")
                .element(Elementary.FIRE).rank(Rank.COMMON).build();
